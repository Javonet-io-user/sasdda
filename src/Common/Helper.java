...
public class Helper {
  ...
  public static <T> T Convert(Object o, Class<T> type) {
    try {
      ...
      if (type == NObject.class) return (T) o;
  ...
  public static <T> NType getGetObjectName(T arg1) throws JavonetException {
    return Javonet.getType(
        Javonet.getType("JavaImportGenerator.WrapperHelper").invoke("GetObjectName", arg1));
  }
  ...
  public static <T> T[] ConvertNObjectToDestinationType(NObject[] nObjects, Class<T[]> arrayType) {
    try {
      ...
      for (int i = 0; i < nObjects.length; i++)
        newArray[i] =
            (T)
                Class.forName(elementType.getName())
                    .getConstructor(NObject.class)
                    .newInstance(nObjects[i]);
      return newArray;
    } catch (Exception e) {
      e.printStackTrace();
      return null;
  ...
  public static <T> T ConvertToConcreteInterfaceImplementation(NObject nObject) {
    try {
      ...
      if (Class.forName(fullName) != null) return (T) nObject;
